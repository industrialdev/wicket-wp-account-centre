...
class Helper
{
     ...
     * @return string
     */
    public static function get_relation_type_from_array($array, $allowed_relations = ['AND', 'OR'], $relation_key = 'relation')
    {
        $allowed_relations = array_values($allowed_relations);
        $allowed_relations = array_map('strtoupper', $allowed_relations);
        $relation = $allowed_relations[0] ?? '';
        ...
        if (strpos($url, $dir['baseurl'] . '/') !== false) {
            ...
            if ($query->have_posts()) {
                foreach ($query->posts as $post_id) {
                    ...
                    $sizes = isset($meta['sizes']) && !empty($meta['sizes']) ? $meta['sizes'] : [];
