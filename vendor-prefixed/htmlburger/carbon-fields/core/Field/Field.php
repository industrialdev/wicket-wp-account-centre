...
class Field implements Datastore_Holder_Interface
{
     ...
     * @var array<string>
     */
    protected static $activated_field_types = [];
    ...
    public static function factory($raw_type, $name, $label = null)
    {
        ...
        if (Carbon_Fields::has($type, 'fields')) {
            return Carbon_Fields::resolve_with_arguments($type, [
                'type' => $type,
                'name' => $name,
                'label' => $label,
            ], 'fields');
    ...
    public function activate()
    {
        ...
        add_action('admin_print_footer_scripts', [static::class, 'admin_hook_scripts'], 5);
        add_action('admin_print_footer_scripts', [static::class, 'admin_hook_styles'], 5);
    ...
    public static function activate_field_type($class_name)
    {
        ...
        add_action('admin_print_footer_scripts', [$class_name, 'admin_enqueue_scripts'], 5);
        call_user_func([$class_name, 'field_type_activated']);
    ...
    {
        $hierarchy_index = (!empty($hierarchy_index)) ? $hierarchy_index : [];
    ...
    public function is_simple_root_field()
    {
        ...
        return
            empty($hierarchy)
            &&
            in_array($this->get_value_set()->get_type(), [Value_Set::TYPE_SINGLE_VALUE, Value_Set::TYPE_MULTIPLE_PROPERTIES]);
    }
    ...
    public function set_attribute($name, $value = '')
    {
        ...
        if (!$is_data_attribute && !in_array($name, $this->allowed_attributes)) {
            Incorrect_Syntax_Exception::raise('Only the following attributes are allowed: ' . implode(', ', array_merge($this->allowed_attributes, ['data-*'])));
    ...
    {
        $allowed_operators = ['=', '!=', '>', '>=', '<', '<=', 'IN', 'NOT IN', 'INCLUDES', 'EXCLUDES'];
