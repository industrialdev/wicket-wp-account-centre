...
class Complex_Field extends Field
{
    ...
    {
        $hierarchy = array_merge($this->get_hierarchy(), [$this->get_base_name()]);
    ...
    public function add_fields()
    {
        ...
        $reserved_names = [Value_Set::VALUE_PROPERTY, static::TYPE_PROPERTY];
        foreach ($fields as $field) {
            /** @var Field $field */
    ...
    {
        $clone = clone $field;
        $clone->set_hierarchy(array_merge($parent_field->get_hierarchy(), [$parent_field->get_base_name()]));
        $clone->set_hierarchy_index(array_merge($parent_field->get_hierarchy_index(), [$group_index]));
    ...
    protected function get_prefilled_groups($groups, $value_tree)
    {
        ...
        foreach ($value_tree as $group_index => $value) {
            ...
            $fields[$group_index] = [Value_Set::VALUE_PROPERTY => $group->get_name()] + $this->get_prefilled_group_fields($group_fields, $group_values, $group_index);
        }
    ...
    public function set_value_from_input($input)
    {
        ...
        foreach ($input_groups as $values) {
            ...
            $value_group = [Value_Set::VALUE_PROPERTY => $values[Value_Set::VALUE_PROPERTY]];
    ...
    protected function external_to_internal_value($value)
    {
        ...
        $value = array_map([$this, 'external_to_internal_value'], $value);
    ...
    public function set_value($value)
    {
        ...
        foreach ($value as $values) {
            $groups[] = $values[Value_Set::VALUE_PROPERTY] ?? Group_Field::DEFAULT_GROUP_NAME;
        }
    ...
    public function to_json($load)
    {
        ...
        $complex_data = array_merge($complex_data, [
