...
class Association_Field extends Field
{
    ...
    public function __construct($type, $name, $label)
    {
        ...
        $this->set_value_set(new Value_Set(Value_Set::TYPE_VALUE_SET, ['type' => '', 'subtype' => '', 'id' => 0]));
    ...
    protected function value_string_to_property_array($value_string)
    {
        ...
        $type = $value_pieces[0] ?? 'post';
        $subtype = $value_pieces[1] ?? 'post';
        $id = $value_pieces[2] ?? 0;
     ...
     * @return string
     */
    public function get_post_options_sql($args = [])
    {
        $type = $args['type'];
        ...
        $args = apply_filters($filter_name, [
            'post_type'        => $post_type,
            'posts_per_page'   => 1,
            'fields'           => 'ids',
        ...
        add_filter('posts_fields_request', [$this, 'get_post_options_sql_select_clause']);
        ...
        remove_filter('posts_fields_request', [$this, 'get_post_options_sql_select_clause']);
     ...
     * @return string
     */
    public function get_term_options_sql($args = [])
    {
        $type = $args['type'];
        ...
        add_filter('get_terms_fields', [$this, 'get_term_options_sql_select_clause']);
        add_filter('terms_clauses', [$this, 'get_term_options_sql_clauses']);

        $terms_query = new WP_Term_Query($args);

        remove_filter('get_terms_fields', [$this, 'get_term_options_sql_select_clause']);
        remove_filter('terms_clauses', [$this, 'get_term_options_sql_clauses']);
    ...
    {
        return ['`t`.`term_id` AS `ID`', '`t`.`name` AS `title`', '\'term\' as `type`', '`tt`.`taxonomy` AS `subtype`'];
    }
     ...
     * @return string
     */
    public function get_user_options_sql($args = [])
    {
        global $wpdb;
     ...
     * @return string
     */
    public function get_comment_options_sql($args = [])
    {
        $type = $args['type'];
        $search_term = $args['term'];
        ...
        add_filter('comments_clauses', [$this, 'get_comments_clauses']);

        $comments_query = new WP_Comment_Query();
        $comments_query->query($args);

        remove_filter('comments_clauses', [$this, 'get_comments_clauses']);
    ...
    public function get_title_by_type($id, $type, $subtype = '')
    {
        ...
        $callable = [$this->wp_toolset, $method];
    ...
    public function format_user_option($data)
    {
        return [
            ...
            'thumbnail'  => get_avatar_url($data->ID, ['size' => 150]),
            'type'       => 'user',
            'subtype'    => 'user',
