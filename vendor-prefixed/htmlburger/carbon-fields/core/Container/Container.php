...
abstract class Container implements Datastore_Holder_Interface
{
     ...
     *
     * @var Fulfillable\Translator\Translator
     */
    ...
    public static function factory($raw_type, $id, $name = '')
    {
        ...
        if (Carbon_Fields::has($type, 'containers')) {
            $container = Carbon_Fields::resolve_with_arguments($type, [
                'id' => $id,
                'name' => $name,
                'type' => $type,
            ], 'containers');
     ...
     * @param Fulfillable\Translator\Translator $condition_translator
     */
    ...
    {
        $param = func_get_args();
        if (call_user_func_array([$this, '_is_valid_save'], $param)) {
            call_user_func_array([$this, 'save'], $param);
        }
    }
    ...
    final protected function _is_valid_save()
    {
        ...
        $is_valid_save = call_user_func_array([$this, 'is_valid_save'], $params);
    ...
    public function _attach()
    {
        ...
        $param = func_get_args();
        call_user_func_array([$this, 'attach'], $param);
    ...
    public function get_field_by_name($field_name)
    {
        ...
        while (!empty($hierarchy_left)) {
            ...
            $segment_group_index = $segment_pieces['group_index'] ?? 0;
            $segment_group_name = $segment_pieces['group_name'] ?? Group_Field::DEFAULT_GROUP_NAME;
    ...
    {
        $conditions = $this->condition_collection->evaluate($this->get_condition_types(true), $this->get_environment_for_request(), ['CUSTOM']);
    ...
    {
        call_user_func_array([$this->condition_collection, 'where'], func_get_args());
    ...
    {
        call_user_func_array([$this->condition_collection, 'or_where'], func_get_args());
