...
class Fulfillable_Collection implements Fulfillable
{
     ...
     * @var array<string>
     */
    protected $supported_fulfillable_comparisons = ['AND', 'OR'];
    ...
    protected function propagate_condition_type_list()
    {
        ...
        foreach ($fulfillables as $fulfillable) {
            if ($fulfillable['fulfillable'] instanceof self) {
    ...
    public function filter($condition_whitelist)
    {
        ...
        foreach ($fulfillables as $fulfillable_tuple) {
            ...
            if ($fulfillable instanceof self) {
     ...
     * @param  array|bool          $environment Environment array or a boolean value to force on conditions
     * @param  array<string>          $comparison_operators Array of comparison operators to evaluate regardless of condition type
     * @param  bool                $condition_types_blacklist Whether the condition list should act as a blacklist
     * @param  bool                $comparison_operators_blacklist Whether the comparison operators list should act as a blacklist
     * @return Fulfillable_Collection
     */
    public function evaluate($condition_types, $environment, $comparison_operators = [], $condition_types_blacklist = false, $comparison_operators_blacklist = false)
    {
        $fulfillables = $this->get_fulfillables();
