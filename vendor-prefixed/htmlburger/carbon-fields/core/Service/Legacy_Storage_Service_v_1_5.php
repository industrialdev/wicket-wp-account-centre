...
class Legacy_Storage_Service_v_1_5 extends Service
{
     ...
     * @var array
     */
    protected $map_keys = ['lat', 'lng', 'zoom', 'address'];
    ...
    {
        add_filter('carbon_fields_datastore_storage_array', [$this, 'filter_storage_array'], 10, 3);
    }
    ...
    {
        remove_filter('carbon_fields_datastore_storage_array', [$this, 'filter_storage_array'], 10);
    }
    ...
    protected function get_legacy_storage_array_from_database(Container $container, Datastore_Interface $datastore)
    {
        ...
        if ($table['table_id_column'] && !$datastore->get_object_id()) {
            return []; // bail as we have an ID column but no ID to compare wicketacc_with ( e.g. we are in a "create" view )
        }
        ...
        if (empty($comparisons)) {
            return []; // no comparisons to fetch with
        }
    ...
    public function get_legacy_storage_array(Datastore_Interface $datastore)
    {
        ...
        if (!$container) {
            return []; // unhandled datastore type or no registered containers
        }
    ...
    {
        $value = $legacy_storage_array[$key] ?? '';
    ...
    public function get_storage_array_for_datastore(Datastore_Interface $datastore)
    {
        ...
        if (empty($legacy_storage_array)) {
            return []; // no migration data found
        }
