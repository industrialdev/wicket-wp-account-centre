...
class Meta_Query_Service extends Service
{
    ...
    {
        add_filter('get_meta_sql', [$this, 'filter_get_meta_sql'], 10, 1);

        add_action('pre_get_posts', [$this, 'hook_pre_get_posts']);
        add_action('pre_get_terms', [$this, 'hook_pre_get_terms']);
        add_action('pre_get_users', [$this, 'hook_pre_get_users']);
    }
    ...
    {
        remove_filter('get_meta_sql', [$this, 'filter_get_meta_sql']);

        remove_action('pre_get_posts', [$this, 'hook_pre_get_posts']);
        remove_action('pre_get_terms', [$this, 'hook_pre_get_terms']);
        remove_action('pre_get_users', [$this, 'hook_pre_get_users']);
    }
    ...
    protected function filter_meta_query_array($condition, $container_type)
    {
        ...
        $property = $condition['carbon_field_property'] ?? Value_Set::VALUE_PROPERTY;
        ...
        if ($field !== null) {
            $storage_key = $this->key_toolset->get_storage_key_with_index_wildcards(
                ...
                array_merge($field->get_hierarchy(), [$field->get_base_name()]),
                $property
            );
    ...
    {
        $meta_query = !empty($query->query_vars['meta_query']) ? $query->query_vars['meta_query'] : [];
        if (!empty($meta_query)) {
