...
namespace WicketAcc\GuzzleHttp\Handler;

use WicketAcc\GuzzleHttp\Exception\ConnectException;
use WicketAcc\GuzzleHttp\Exception\RequestException;
use WicketAcc\GuzzleHttp\Promise as P;
use WicketAcc\GuzzleHttp\Promise\FulfilledPromise;
use WicketAcc\GuzzleHttp\Promise\PromiseInterface;
use WicketAcc\GuzzleHttp\Psr7\LazyOpenStream;
use WicketAcc\GuzzleHttp\TransferStats;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \WicketAcc\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \WicketAcc\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \WicketAcc\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \WicketAcc\GuzzleHttp\Psr7\Utils::streamFor($sink);
