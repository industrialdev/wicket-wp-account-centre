...
class Factory
{
    ...
    public static function create($data, $related = false)
    {
        ...
        $className = implode('\\', [__NAMESPACE__, Str::studly($data['type'])]);

        if (!class_exists($className)) {
            $className = implode('\\', [__NAMESPACE__, 'Base']);
        }
        ...
        if ($related && array_key_exists('relationships', $data)) {
            foreach ($data['relationships'] as $k => $related) {
                if (array_key_exists('data', $related) && !empty($related['data'])) {
                    ...
                    if (isset($reldata['id'])) {
                        $entity->addRelationship($k, self::create($reldata));
                    } else {
                        foreach ($reldata as $relation) {
                            $entity->addRelationship($k, self::create($relation));
                        }
                    }
