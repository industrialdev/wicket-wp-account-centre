...
foreach ($config['original_files'] as $originalFilePath) {
    $translationFilePaths = wicketacc_findTranslationFiles($originalFilePath, $config['locale_to_analyze']);
    $translationStatus = wicketacc_calculateTranslationStatus($originalFilePath, $translationFilePaths);
    ...
    wicketacc_printTranslationStatus($originalFilePath, $translationStatus, $config['verbose_output'], $config['include_completed_languages']);
}

exit($totalTranslationMismatches > 0 ? 1 : 0);

function wicketacc_findTranslationFiles($originalFilePath, $localeToAnalyze): array
{
    $translations = [];
...
function wicketacc_calculateTranslationStatus($originalFilePath, $translationFilePaths): array
{
    $translationStatus = [];
    $allTranslationKeys = wicketacc_extractTranslationKeys($originalFilePath);

    foreach ($translationFilePaths as $locale => $translationPath) {
        $translatedKeys = wicketacc_extractTranslationKeys($translationPath);
        $missingKeys = array_diff_key($allTranslationKeys, $translatedKeys);
        $mismatches = wicketacc_findTransUnitMismatches($allTranslationKeys, $translatedKeys);
        ...
        $translationStatus[$locale]['is_completed'] = wicketacc_isTranslationCompleted($translationStatus[$locale]);
    }

    return $translationStatus;
}

function wicketacc_isTranslationCompleted(array $translationStatus): bool
{
    return $translationStatus['total'] === $translationStatus['translated'] && 0 === count($translationStatus['mismatches']);
}

function wicketacc_printTranslationStatus($originalFilePath, $translationStatus, $verboseOutput, $includeCompletedLanguages)
{
...
function wicketacc_extractTranslationKeys($filePath): array
{
    $translationKeys = [];
 ...
 */
function wicketacc_findTransUnitMismatches(array $baseTranslationKeys, array $translatedKeys): array
{
    $mismatches = [];
...
function wicketacc_textColorGreen()
{
    echo "\033[32m";
}

function wicketacc_textColorRed()
{
    echo "\033[31m";
}

function wicketacc_textColorNormal()
{
    echo "\033[0m";
}
