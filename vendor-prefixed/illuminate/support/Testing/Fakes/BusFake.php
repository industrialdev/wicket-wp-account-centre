...
use WicketAcc\Illuminate\Contracts\Bus\QueueingDispatcher;
...
class BusFake implements Fake, QueueingDispatcher
{
     ...
     *
     * @var \WicketAcc\Illuminate\Contracts\Bus\QueueingDispatcher
     */
    public $dispatcher;
     ...
     *
     * @param  \WicketAcc\Illuminate\Contracts\Bus\QueueingDispatcher  $dispatcher
    ...
    {
        return wicketacc_tap(clone $job, function ($job) {
     ...
     * @return \WicketAcc\Illuminate\Support\Testing\Fakes\ChainedBatchTruthTest
     */
    ...
    public function dispatched($command, $callback = null)
    {
        ...
        return wicketacc_collect($this->commands[$command])->filter(fn ($command) => $callback($command));
    }
    ...
    public function dispatchedSync(string $command, $callback = null)
    {
        ...
        return wicketacc_collect($this->commandsSync[$command])->filter(fn ($command) => $callback($command));
    }
    ...
    public function dispatchedAfterResponse(string $command, $callback = null)
    {
        ...
        return wicketacc_collect($this->commandsAfterResponse[$command])->filter(fn ($command) => $callback($command));
    }
    ...
    public function batched(callable $callback)
    {
        ...
        return wicketacc_collect($this->batches)->filter(fn ($batch) => $callback($batch));
    }
     ...
     *
     * @param  \WicketAcc\Illuminate\Support\Collection|array  $jobs
    ...
    protected function shouldFakeJob($command)
    {
        ...
        return wicketacc_collect($this->jobsToFake)
    ...
    {
        return wicketacc_collect($this->jobsToDispatch)
            ->filter(function ($job) use ($command) {
                return $job instanceof Closure
                    ? $job($command)
                    : $job === get_class($command);
