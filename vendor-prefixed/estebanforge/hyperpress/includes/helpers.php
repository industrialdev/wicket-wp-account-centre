 ...
 * @return string
 */
function wicketacc_hxwp_api_url($template_path = '')
{
 ...
 * @return void
 */
function wicketacc_hxwp_send_header_response($nonce = null, $data = [], $action = null)
{
    if (!isset($nonce)) {
        wicketacc_hxwp_die('Nonce not provided.');
    }

    if (isset($nonce) && !wp_verify_nonce($nonce, 'hxwp_nonce')) {
        wicketacc_hxwp_die('Nonce verification failed.');
    }
 ...
 * @return void
 */
function wicketacc_hxwp_die($message = '', $display_error = false)
{
    // Send our response
    if (!headers_sent()) {
        status_header(200);
 ...
 * @return bool
 */
function wicketacc_hxwp_validate_request($hxvals, $action = null)
{
    // Secure it.
