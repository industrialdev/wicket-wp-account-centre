...
class AdminSettings extends \WicketAcc\WicketAcc
{
    ...
    /**
     * Reorder the 'My Account' submenu items to place both options pages third and fourth.
     *
     * This works by finding our options pages in the global $submenu array,
     * removing them, and then re-inserting them at the desired positions.
     */
    public function reorderAccSubmenu()
    {
        ...
        $carbon_options_slug =
            "crb_carbon_fields_container_wicket_acc_options.php";
        $hyperfields_options_slug = "wicket-acc-options";
        ...
        $carbon_options_item = null;
        $hyperfields_options_item = null;

        // Find and remove both options pages
        foreach ($menu_items as $key => $item) {
            if ($item[2] === $carbon_options_slug) {
                $carbon_options_item = $item;
                unset($menu_items[$key]);
            } elseif ($item[2] === $hyperfields_options_slug) {
                $hyperfields_options_item = $item;
                unset($menu_items[$key]);
            }
        }

        // Rebuild the menu with our items at the correct positions
        $new_menu = array_values($menu_items);

        // Insert Carbon Fields options at third position (index 2)
        if ($carbon_options_item) {
            array_splice($new_menu, 2, 0, [$carbon_options_item]);
        }

        // Insert HyperFields options at fourth position (index 3)
        if ($hyperfields_options_item) {
            array_splice($new_menu, 3, 0, [$hyperfields_options_item]);
        }
    ...
    public function accCustomAdminLinks()
    {
        ...
        $acc_banner_enabled = WACC()->getOption(
    ...
    public function enqueueAdminAssets()
    {
        ...
        wp_enqueue_script(
            "wicket-acc-admin",
            WICKET_ACC_URL . "assets/js/wicket-acc-admin-main.js",
            ["jquery"],
            "1.0",
            true,
        );
        wp_enqueue_style(
            "wicket-acc-admin",
            WICKET_ACC_URL . "assets/css/wicket-acc-admin-main.css",
            false,
            "1.0",
        );
    }
    ...
    public function checkAcfJsonFolderPermissions()
    {
        ...
        if (!is_writable($acf_json_folder)) {
            echo '<div class="notice notice-error"><p><strong>ACF JSON folder not writable</strong></p><p>The ' .
                esc_html($acf_json_folder) .
                " folder is not writable. Please make sure the folder is writable by the server.</p><p>Not solving this issue, will result in ACF fields not being saved at plugin level and will not be visible on other sites backend.</p></div>";
        }
    }
}
