								...
								schemas: [ // If schemas are not provided, the widget defaults to show all schemas.
									...
									if ($ai_schema) :
										foreach ($ai_schema as $schema) : ?> {
												id: '<?php echo ($environment == 'prod') ? $schema['schema_id_prod'] : $schema['schema_id_stage']; ?>',
												<?php if ($schema['schema_read_only']) { ?> resourceId: '<?php echo ($environment == 'prod') ? $schema['read_only_schema_id_prod'] : $schema['read_only_schema_id_stage']; ?>'
												<?php } ?>
											},
									<?php endforeach;
									endif; ?>
