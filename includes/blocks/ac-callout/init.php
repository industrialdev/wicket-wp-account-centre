...
class init extends Blocks
{
            ...
            case 'renewal':
                ...
                if (!class_exists('\\Wicket_Memberships\\Wicket_Memberships')) {
                    ...
                    if (!empty($_ENV['WICKET_MEMBERSHIPS_DEBUG_ACC'])) {
                        ...
                        $all_hide_classes = array_merge(['.acc_hide_mship_any'], $tier_hide_classes);
                        $hide_classes_html = '';
                        foreach ($all_hide_classes as $index => $class_name) {
                            if ($index > 0) {
                                $hide_classes_html .= ', ';
                            }
                            $hide_classes_html .= '<code style="background-color:#ccc;font-size:10pt;"> ' . esc_html($class_name) . ' </code>';
                        }
                        echo '<div class="acc-callout-debug-info" style="padding: 1rem; margin: 1rem; border: solid 2px #ccc; border-radius: 5px;"><p style="font-size: 1rem;">For testing callouts add <code style="background-color:#ccc;font-size:10pt;"> ?wicket_wp_membership_debug_days=123 </code>&nbsp;to see what callouts would appear in 123 days.</p>';
                        echo '<p style="font-size: 1rem;">You can add the following classes to any element on this page to hide when an active or delayed status membership exists for the user:<br/>' . $hide_classes_html . '</p></div>';
                        echo '<script>(function(){if(window.__accCalloutDebugCopyInit){return;}window.__accCalloutDebugCopyInit=true;if(!document.getElementById("acc-callout-debug-copy-style")){var style=document.createElement("style");style.id="acc-callout-debug-copy-style";style.textContent=".acc-callout-debug-info code{position:relative;display:inline-block;transition:background-color .25s ease}.acc-callout-debug-info code.acc-copy-active{background-color:#dff5e5!important;animation:accCopyFlash .7s ease}@keyframes accCopyFlash{0%{box-shadow:0 0 0 0 rgba(10,125,54,.5)}100%{box-shadow:0 0 0 8px rgba(10,125,54,0)}}.acc-callout-debug-info code.acc-copy-tooltip::after{content:\"Copied to clipboard\";position:absolute;left:50%;transform:translateX(-50%);bottom:calc(100% + 6px);background:#0a7d36;color:#fff;font-size:11px;line-height:1.2;padding:4px 7px;border-radius:4px;white-space:nowrap;z-index:20;pointer-events:none;opacity:1}.acc-callout-debug-info code.acc-copy-tooltip::before{content:\"\";position:absolute;left:50%;transform:translateX(-50%);bottom:calc(100% + 1px);border:5px solid transparent;border-top-color:#0a7d36;z-index:20;pointer-events:none}";document.head.appendChild(style);}var showFeedback=function(code){code.classList.remove("acc-copy-active","acc-copy-tooltip");void code.offsetWidth;code.classList.add("acc-copy-active","acc-copy-tooltip");if(code.__accCopyTimeout){clearTimeout(code.__accCopyTimeout);}code.__accCopyTimeout=setTimeout(function(){code.classList.remove("acc-copy-active","acc-copy-tooltip");},1200);};document.querySelectorAll(".acc-callout-debug-info code").forEach(function(el){el.style.cursor="pointer";el.title="Click to copy";});document.addEventListener("click",function(event){var code=event.target.closest(".acc-callout-debug-info code");if(!code){return;}var text=code.textContent.trim();if(!text){return;}var onSuccess=function(){showFeedback(code);};if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(text).then(onSuccess);}else{var input=document.createElement("textarea");input.value=text;document.body.appendChild(input);input.select();try{document.execCommand("copy");onSuccess();}catch(e){}document.body.removeChild(input);}});})();</script>';
                    }
    ...
    private function render_confirmation_renewal_callout(array $callout): void
    {
        ...
        $success_label = __('Renewal confirmed successfully.', 'wicket-acc');
        ...
        <div class="<?php echo esc_attr(implode(' ', $wrapper_classes)); ?>">
            ...
            >
                <div class="<?php echo $is_wicket_theme_v2 ? 'component-card-call-out__links' : 'flex flex-col @md:flex-row gap-2'; ?>">
                    <?php
                    get_component('button', [
                        ...
                        'atts'     => ['data-wicket-acc-confirmation-renewal-button' => true],
                    ]);
                    ?>
                </div>
