...
function init( $block = [], $is_preview ) {
  ...
  if( empty( $registered_action ) ) {
    $registered_action = array( "rsvp_to_event", "registered_for_an_event", "attended_an_event" );
  }
  //echo "<pre>"; var_dump($registered_action);exit;
  ...
  if( !empty( $_REQUEST['num_results'] ) ) {
    $num_results = sanitize_text_field( $_REQUEST['num_results'] );
  }
...
function display_events( $event_touchpoints, $display_type, $num_results ) {
      if ($event_touchpoints['data']):		
				...
				?>
					<?php if ($tp['attributes']['code'] == 'registered_for_an_event'): ?>
						...
						<?php if(isset($tp['attributes']['data']['start_date'])): ?>
							<div class='touchpoint_row mb-3 flex'>
                  ...
                  <div class="calendar_content">
										...
										if (isset($tp['attributes']['data']['event_additional_fields'])):
											foreach ($tp['attributes']['data']['event_additional_fields'] as $meta):
												foreach ($meta as $label => $value): 
													?>
													<div class='event_landing_custom_field'><strong><?php echo $label; ?>: </strong>
					...
					<?php if($counter == $num_results): ?>
						<p><a href='#' class='touchpoint_show_more_cta' id='touchpoint_show_more_cta' @click.prevent="show = !show" x-show="!show">Show More</a></p>
				...
				echo '<p>You do not have any '. $display_type .' events at this time</p>';
			endif;
}