...
class ajax extends init
{
    ...
    {
        // If action != wicket_ac_touchpoint_moodle_results, return
        if (!isset($_POST['action']) || $_POST['action'] != 'wicket_ac_touchpoint_moodle_results') {
            return false;
        }

        // Verify nonce
        $nonce_validation = check_ajax_referer('wicket_ac_touchpoint_moodle_results', 'security');
