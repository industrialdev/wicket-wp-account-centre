...
class AdminSettings extends WicketAcc
{
	...
	{
		// add_action('admin_enqueue_scripts', [$this, 'acc_admin_enqueue_scripts']);
		add_action('admin_menu', [$this, 'acc_admin_menu']);
		add_action('add_meta_boxes', [$this, 'acc_admin_meta_box']);
		add_action('save_post_wicket_acc', [$this, 'acc_admin_save_metabox']);
		add_action('admin_init', [$this, 'acc_admin_settings_fields']);
		add_filter('manage_edit-wicket_acc_columns', [$this, 'acc_admin_edit_columns']);
		add_action('admin_head', [$this, 'acc_admin_columns_width']);
		add_action('manage_wicket_acc_posts_custom_column', [$this, 'acc_admin_manage_columns'], 10, 2);
	}
	 ...
	 * @return void
	 */
	public function acc_admin_enqueue_scripts()
	{

		wp_enqueue_script('wicket_acc_admin', plugins_url('../assets/js/wicket_acc_admin.js', __FILE__), ['jquery'], '1.0', false);

		wp_enqueue_style('wicket_acc_admin', plugins_url('../assets/css/wicket_acc_admin.css', __FILE__), [], '1.0');
...
<?php
	 ...
	 */
	public function acc_admin_settings_fields()
	{
	 ...
	 */
	public function acc_admin_save_metabox($post_id)
	{
		...
		if (isset($_POST['wicket_acc_user_role'])) {
			...
			update_post_meta($post_id, 'wicket_acc_user_role', []);
		}
		...
		if (isset($_POST['wicket_acc_group_child'])) {
			...
			update_post_meta($post_id, 'wicket_acc_group_child', wp_json_encode([]));
		}
		...
		if (isset($_POST['wicket_acc_group_default_filed'])) {
			...
			if (is_array($child_array)) {
				...
				update_post_meta($post_id, 'wicket_acc_group_default_filed', wp_json_encode([]));
			}
		} else {
			update_post_meta($post_id, 'wicket_acc_group_default_filed', wp_json_encode([]));
		}
	 ...
	 * @return array
	 */
	public function acc_admin_edit_columns($columns)
	{
	 ...
	 * @return void
	 */
	public function acc_admin_columns_width()
	{
	 ...
	 */
	public function acc_admin_manage_columns($column, $post_id)
	{
		global $post;
