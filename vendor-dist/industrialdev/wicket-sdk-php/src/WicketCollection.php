...
use WicketAcc\Illuminate\Contracts\Pagination\LengthAwarePaginator;
use WicketAcc\Illuminate\Contracts\Pagination\Presenter;
	...
	extends Collection
{
	 ...
	 * @param \WicketAcc\Wicket\Client|null $client
	 */
	public function __construct($response, Client $client = null)
	{
		$ent_list = array_map(function ($ent) {
			return Base::fromJsonAPI($ent);
		}, wicketacc_data_get($response, 'data', []));

		parent::__construct($ent_list);

		$this->self = wicketacc_data_get($response, 'links.self', null);
		$this->next = wicketacc_data_get($response, 'links.next', null);
		$this->prev = wicketacc_data_get($response, 'links.prev', null);
		$this->last = wicketacc_data_get($response, 'links.last', null);
		$this->first = wicketacc_data_get($response, 'links.first', null);
		$this->total_items = wicketacc_data_get($response, 'meta.page.total_items', null);
		$this->total_pages = wicketacc_data_get($response, 'meta.page.total_pages', null);
		$this->number = wicketacc_data_get($response, 'meta.page.number', null);
		$this->size = wicketacc_data_get($response, 'meta.page.size', null);
		$this->client = $client;
	}

	/**
	 * @param $url
	 * @return array|false|\WicketAcc\Wicket\WicketCollection If the response has a `data` block, else the reponse.
	 */
