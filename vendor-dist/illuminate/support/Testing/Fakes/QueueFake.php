...
class QueueFake extends QueueManager implements Fake, Queue
{
    ...
    public function assertPushedWithChain($job, $expectedChain = [], $callback = null)
    {
        ...
        PHPUnit::assertTrue(
            wicketacc_collect($expectedChain)->isNotEmpty(),
    ...
    {
        $chain = wicketacc_collect($expectedChain)->map(fn ($job) => serialize($job))->all();
    ...
    protected function assertPushedWithChainOfClasses($job, $expectedChain, $callback)
    {
        $matching = $this->pushed($job, $callback)->map->chained->map(function ($chain) {
            return wicketacc_collect($chain)->map(function ($job) {
    ...
    public function pushed($job, $callback = null)
    {
        ...
        return wicketacc_collect($this->jobs[$job])->filter(
    ...
    {
        return wicketacc_collect($this->jobs)->flatten(1)->filter(
